...
## 使用证书管理器 CSI 驱动程序启用 Pods 的 MTLS

实验 [Container Storage Interface (CSI) 驱动](https://github.com/jetstack/cert-manager-csi) 已经建立，以方便`Pods`运行 MTLS 通过使用证书管理器的群集内.
...
使用密钥的完整列表[从代码](https://github.com/jetstack/cert-manager/blob/d4a743f91ab1fdfefd21354c82aaa311888afa02/pkg/apis/certmanager/v1alpha2/types.go#L169-L193)可用.

| 属性                                                                | 默认                                 | 例                               |
| ------------------------------------------------------------------- | ------------------------------------ | -------------------------------- |
| `csi.cert-manager.io/issuer-name`                                   |                                      | `ca-issuer`                      |
| 发行人名称签名证书请求.                                             |                                      |                                  |
| `csi.cert-manager.io/issuer-kind`                                   | `Issuer`                             | `ClusterIssuer`                  |
| 发行人那种对证书签名请求.                                           |                                      |                                  |
| `csi.cert-manager.io/issuer-group`                                  | `cert-manager.io`                    | `out.of.tree.foo`                |
| 组名发行人所属.                                                     |                                      |                                  |
| `csi.cert-manager.io/common-name`                                   |                                      | `my-cert.foo`                    |
| 证书的通用名称.                                                     |                                      |                                  |
| `csi.cert-manager.io/key-usages`                                    | `digital signature,key encipherment` | `signing,timestamping`           |
| 证书密钥用法.                                                       |                                      |                                  |
| `csi.cert-manager.io/dns-names`                                     |                                      | `a.b.foo.com,c.d.foo.com`        |
| 该证书将请求 DNS 名称. 至少有一个 DNS 名称, IP 或 URI 名称必须存在. |                                      |                                  |
| `csi.cert-manager.io/ip-sans`                                       |                                      | `192.0.0.1,192.0.0.2`            |
| 该证书将被要求对 IP 地址.                                           |                                      |                                  |
| `csi.cert-manager.io/uri-sans`                                      |                                      | `spiffe://foo.bar.cluster.local` |
| URI 名称的证书将请求.                                               |                                      |                                  |
| `csi.cert-manager.io/duration`                                      | `720h`                               | `1880h`                          |
| 请求历时签名证书的有效期为.                                         |                                      |                                  |
| `csi.cert-manager.io/is-ca`                                         | `false`                              | `true`                           |
| 纪念证书作为证书颁发机构.                                           |                                      |                                  |
| `csi.cert-manager.io/certificate-file`                              | `crt.pem`                            | `bar/foo.crt`                    |
| 文件名存储证书文件的.                                               |                                      |                                  |
| `csi.cert-manager.io/privatekey-file`                               | `key.pem`                            | `bar/foo.key`                    |
| 文件名存储密钥文件的.                                               |                                      |                                  |
| `csi.cert-manager.io/renew-before`                                  | `$CERT_DURATION/3`                   | `72h`                            |
| 时间续约期满之前证书. 默认为第三请求的持续时间.                     |                                      |                                  |
| `csi.cert-manager.io/disable-auto-renew`                            | `false`                              | `true`                           |
| 从安装到`pod`更新证书禁用 CSI 驱动.                                 |                                      |                                  |
| `csi.cert-manager.io/reuse-private-key`                             | `false`                              | `true`                           |
| 更新证书时，当再次使用相同的私.                                     |                                      |                                  |
