...
当请求 ACME 证书, cert-manager 将创建 `Order` 和 `Challenges` 完成请求.
因此, 有更多的资源来调查和调试 if there is a problem during the process. You can read more about these resources in the [concepts pages](../../concepts/acme-orders-challenges/).

在您开始在这里你应该看看我们的[一般故障排除指南](../troubleshooting/)

## 1. 故障排除订单

当我们运行在`CertificateRequest`资源描述 我们看到，已经创建了一个`Order`:
...
为了确定 为什么一个 ACME 订单未完成, we can debug using the `Challenge` resources that cert-manager has created.
...
## 2020 年 3 月 Let's Encrypt CAA 复查错误
