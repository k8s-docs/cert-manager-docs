...
SelfSigned Issuer 并不代表证书颁发机构, 而是表示证书将通过“self signing”使用给定的私钥签名.
这意味着产生的证书所提供的私钥将用来签署自己的证书.

这个 Issuer 类型是对于一些私人密钥基础设施 (PKI)提供引导 CA 证书密钥对或自行创建简单证书非常有用.
因为除了本身没有 CA 签名者，客户消费这些证书都没有办法信任此证书, 正因为如此将被迫信任证书.
