...
type: "docs"
description: >
  [kube-lego](https://github.com/jetstack/kube-lego) 是一个较旧 Jetstack 项目 从 Let's Encrypt 获得 TLS 证书 (或其他 ACME 服务器).
---

由于 cert-managers 发布, kube-lego 也有利于这个项目的逐步弃用.
有一些两者之间的主要区别:
...
## 4. 使用旧 ACME 帐户创建 ACME `ClusterIssuer`
...
## 5. 配置 ingress-shim 使用我们的新 `ClusterIssuer` 默认
...
docs](../../../usage/ingress/), but for now we can just run a `helm upgrade` in order to add a few additional flags. Assuming you've named your
